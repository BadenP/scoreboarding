...
void escritaResultados(){
    for(int i=0; i<unidadesFuncionais.qtdeADD; i++){
        for(int j=0; j<unidadesFuncionais.qtdeMUL; j++){
            if(((unidadesFuncionais.ufAdd[i].fi!=unidadesFuncionais.ufMul[j].fj) || unidadesFuncionais.ufMul[i].rj==0) && unidadesFuncionais.ufAdd[i].fi!=unidadesFuncionais.ufAdd[i].fk){
